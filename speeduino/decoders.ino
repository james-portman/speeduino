...
/* -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Name: Renault trigger pattern code - 44-1-1 with some double teeth
Desc: Strange trigger wheel on some Renaults including a Williams Clio - 2.0 16v with distributor
Note: The pattern is such: 19 teeth, 1 double tooth, 1 gap, 19 teeth, 1 double tooth, 1 gap
Looks like TDC is the falling edge of the 7th tooth after a gap
This would be 44 of the normal teeth size (ignoring the gaps and doubles)
The gaps are the size of 1 missing normal tooth
Each double tooth is the width of 2 normal teeth, but without the small gap which is usually between teeth

Normal teeth side by side:
_/-\_/-|_/-|_/-|_/-\_/-|_/-|_/-|_

A "double" tooth next to normal ones, then a gap and more normal teeth again
_/-|_/-|_/-----\_____/-|_/-|_/-|_

Trigger on rising edge:
the double and gap will look like the size of 3 teeth, the tooth after will look normal size again
essentially it would look like a double gap: 1001
The code for this would probably be easier, just look for a double gap, but this is leaving it longer without running the trigger code - 3 normal teeth worth of time

Trigger on failling edge:
The double will look like a gap then 1 tooth, the tooth after the gap will again look like a gap then 1 tooth
so essentially it will look to the sensor like 2 gaps: 0101
The code for this might be a bit more complicated but the car would not be running for as long without calling the trigger code - 2 normal teeth time max, should be more accurate/cope with engine speed changing better
Full pattern from TDC: 12 normal teeth, double, gap, 19 normal teeth, double, gap, 7 teeth
11111111111101011111111111111111110101111111
nnnnnnnnnnnnDDGnnnnnnnnnnnnnnnnnnnDDGnnnnnnn
(n=normal, DD=double, G=gap)

*/

void triggerSetup_Renault442()
{
...
void appendTooth_Renault442(byte value)
{
...
void checkPattern_Renault442()
{
  // if we are trying to sync but haven't done a full rev yet then don't even try
  if (currentStatus.hasSync == false && toothCurrentCount < configPage4.triggerTeeth) { return; }
...
void triggerPri_Renault442()
{
  ...
  if (curGap > targetGap) // WAS a gap between the last tooth and this one
  {
      ...
      appendTooth_Renault442(0); // add the gap we saw to our "seen" teeth
      checkPattern_Renault442(); // check if the teeth we've seen match the pattern we are expecting
  ...
  toothCurrentCount++; //Increment the tooth counter
  appendTooth_Renault442(1); // add the tooth we saw to our "seen teeth"
  checkPattern_Renault442(); // check if the teeth we've seen match the pattern we are expecting
...
void triggerSec_Renault442()
{
  // NOT USED - no cam support
}

uint16_t getRPM_Renault442()
{
  // NOT USED - Uses getRPM_missingTooth
}

int getCrankAngle_Renault442()
{
  // NOT USED - uses getCrankAngle_missingTooth
}

void triggerSetEndTeeth_Renault442()
{
